...
#[derive(Clone)]
pub struct ExecutableExpandable
{
...
impl ExecutableExpandable
{
    pub fn new( path : &str, params : Option<String>, cells : Vec<Cell> ) -> Result<Self, Error>
    {
        if cells.len() == 0
        {
            return Err( Error::ExpandableWithoutCells( format!( "ExecutableExpandable: {}", path ) ) )
        }
...
impl Rowable for ExecutableExpandable
{
...
impl ToString for ExecutableExpandable
{
